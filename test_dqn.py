...
def test_short_training():
    ...
    try:
        env = OptimizedPalletEnv()
        vec_env = DummyVecEnv([lambda: OptimizedPalletEnv()])
        model = create_optimized_dqn_model(vec_env, 32)  # Small model for speed
...
def test_evaluation():
    ...
    try:
        ...
        env = OptimizedPalletEnv()
        vec_env = DummyVecEnv([lambda: OptimizedPalletEnv()])
        model = create_optimized_dqn_model(vec_env, 32)
        model.learn(total_timesteps=500)  # Very short training
